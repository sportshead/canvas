 ...
 */
export function detectRectCollision(
    rect1: Rect2D,
    rect2: Rect2D,
    margin: number = 0
): boolean {
 ...
 * @param {Circle2D} c1 First circle
 * @param {Circle2D} c2 Second circle
 * @param {Number} [margin] Margin for hitbox, can be negative
 * @returns {Boolean} Whether or not the circles are colliding.
 */
export function detectCircleCollision(
    ...
    const maxdist = c1.radius + c2.radius + margin;
    return x * x + y * y < maxdist * maxdist;
}
